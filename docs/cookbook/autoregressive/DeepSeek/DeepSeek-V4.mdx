...
**FreeToken prefill expert offload (experimental, SM120)**

FreeToken keeps DeepSeek-V4-Flash experts in pinned host memory and overlaps the
next layer's H2D copy with the current MoE on an SM120 GPU. It is disabled by
default and requires a disaggregated TP1/EP1/DP1 prefill worker with prefill
CUDA graphs and batch overlap disabled. Streaming is implemented at the common
`FusedMoE` offloader boundary instead of inside a MoE runner. It has been
validated with `deep_gemm` and `flashinfer_mxfp4`; both consume the same staged
parameters.

```bash Command
SGLANG_FREETOKEN_PREFILL_OFFLOAD=1 \
SGLANG_FREETOKEN_PREFILL_TARGET_TOKENS=32768 \
SGLANG_FREETOKEN_PREFILL_MAX_WAIT_MS=1000 \
sglang serve <prefill arguments> \
...
The target amortizes a weight sweep; the timeout bounds low-load waiting. Set
either admission value to zero to retain offload without admission coalescing.
